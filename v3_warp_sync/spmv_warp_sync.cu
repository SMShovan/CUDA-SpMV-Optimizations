...
const int ROWS_PER_WARP = 1;  // Each warp processes one row
...
// Kernel for warp-synchronized SpMV implementation
__global__ void spmv_warp_sync_kernel(const double* __restrict__ values,
                                     const int* __restrict__ col_indices,
                                     const int* __restrict__ row_ptr,
                                     const double* __restrict__ x,
                                     double* __restrict__ y,
                                     const int num_rows) {
    // Calculate row index for this warp
    const int warp_id = (blockIdx.x * WARPS_PER_BLOCK) + (threadIdx.x / WARP_SIZE);
    const int row = warp_id * ROWS_PER_WARP;
    const int lane_id = threadIdx.x % WARP_SIZE;
    
    if (row < num_rows) {
        const int row_start = row_ptr[row];
        const int row_end = row_ptr[row + 1];
        const int row_elements = row_end - row_start;
        
        // Initialize partial sum for this thread
        double sum = 0.0;
        
        // Coalesced loading of matrix values and column indices
        for (int j = lane_id; j < row_elements; j += WARP_SIZE) {
            const int idx = row_start + j;
...
int main() {
    ...
    // Warm-up run
    spmv_warp_sync_kernel<<<num_blocks, BLOCK_SIZE>>>(d_matrix.values,
    ...
    std::cout << "Configuration: Warp-sync with " << WARPS_PER_BLOCK << " warps per block" << std::endl;
