...
const int ROWS_PER_BLOCK = 8;  // Number of rows processed by each block

// Kernel for vector SpMV implementation (one warp per row)
__global__ void spmv_vector_kernel(const double* values,
    ...
    if (row < num_rows) {
        ...
        // Threads in a warp cooperatively process elements in the row
...
int main() {
    ...
    int grid_size = (num_rows + ROWS_PER_BLOCK - 1) / ROWS_PER_BLOCK;
    
    // Warm-up run
    spmv_vector_kernel<<<grid_size, BLOCK_SIZE>>>(d_matrix.values,
    ...
    std::cout << "Configuration: " << ROWS_PER_BLOCK << " rows per block" << std::endl;
